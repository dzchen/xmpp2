...
uses
  Classes, SysUtils, blcksock, ssl_openssl, SyncObjs,System.StrUtils;
...
//            end;
            ...
            if (l <> 0) and (ord(inp[l]) >= $80) then
            begin
              i := l;
              while ((i > 0) and (ord(inp[i]) < $C0)) do
                dec(i);

              if ((ord(inp[i]) >= $C0) and (ord(inp[i]) < $E0) and (l-i < 1 )) or
                 ((ord(inp[i]) >= $E0) and (ord(inp[i]) < $F0) and (l-i < 2 )) or
                 ((ord(inp[i]) >= $F0) and (l-i < 3 )) then
              begin
                _remain_utf := RightBStr(inp, l - i + 1);
                inp := LeftBStr(inp, i-1);
              end else
                _remain_utf := '';
            end else
              _remain_utf := '';

            FData:=Utf8ToAnsi(inp);
