unit Xmpp.Client.sasl.Mechanism;

interface
uses
  Element,XmppConnection,XMPPConst,SysUtils,StrUtils
  ,util.Random,IdHash,IdHashMessageDigest,IdCoderMime,Types
  ,StringUtils,System.Generics.Collections,synacode,xmpp.sasl.Mechanism;
type
...
implementation
uses
  XmppClientConnection,xmpp.sasl;
var
...
begin
  ...
  self.XmppClientConnection.Send(xmpp.sasl.TAuth.Create(MTDIGEST_MD5));
end;

procedure TDigestMD5Mechanism.Parse(e: TElement);
var
  c:xmpp.sasl.TChallenge;
  step1:TStep1;
  step2:TStep2;
  r:TResponse;
begin
  if e.IsTagEqual(TChallenge) then
  begin
    c:=TChallenge(e);
...
begin
  ...
  sb.Append('0');
  sb.Append(Password);
  result:=EncodeBase64(sb.ToString);
end;

procedure TPlainMechanism.Init(con: TXmppConnection);
begin
  self.XmppClientConnection:=TXmppClientConnection(con);
  self.XmppClientConnection.Send(xmpp.sasl.TAuth.Create(MTPLAIN));
end;
...
begin
  self.XmppClientConnection:=TXmppClientConnection(con);
  self.XmppClientConnection.Send(xmpp.sasl.TAuth.Create(MTANONYMOUS));
end;
...
initialization
  ...
  TSaslFactory.AddMechanism(xmpp.sasl.Mechanism.TMechanism.GetMechanismName(MTPLAIN),TMechanism);
  TSaslFactory.AddMechanism(xmpp.sasl.Mechanism.TMechanism.GetMechanismName(MTDIGEST_MD5),TDigestMD5Mechanism);
  TSaslFactory.AddMechanism(xmpp.sasl.Mechanism.TMechanism.GetMechanismName(MTANONYMOUS),TMechanism);
  TSaslFactory.AddMechanism(xmpp.sasl.Mechanism.TMechanism.GetMechanismName(MTX_GOOGLE_TOKEN),TMechanism);
finalization
  _table.Clear;
  _table.Free;
