...
uses
  ...
  Windows,
  {$ENDIF}
  Classes, SysUtils, tcpsynapse, xmltag, ExtCtrls,xmpp.iq.RosterItem,xmpp.iq.rosteriq,Xmpp.Iq.BindIq
  ,XmppConstants,jid,xmpp.client.iq,Xmpp.Iq.SessionIq;
...

{
  ...
  TRosterItemEvent=procedure(sender:TObject;rosteritem:TRosterItem) of object;

  TXmppClientConnection=class
  private
    FSocket:TTCPClient;
    FHost,FPort,
    FUser,FPass,
...
//    FOnOfflineMsg:TOfflineMessageEvent;
    ...
    FOnRosterItem:TRosterItemEvent;
    ...
    procedure BindResult(sender:TObject;iq:txmltag;data:Pointer);
    procedure SessionResult(sender:TObject;iq:TXMLTag;data:Pointer);
...
//    property OnOfflineMessage:TOfflineMessageEvent read FOnOfflineMsg write FOnOfflineMsg;
    ...
    property OnRosterItem:TRosterItemEvent read FOnRosterItem write  FOnRosterItem;
  end;
...
destructor TXmppClientConnection.Destroy;
begin
  Logout;
  FTimer.Free;
  FRoomRoster.Free;
...
procedure TXmppClientConnection.SetDefaultVal;
begin
  FAuthd := False;
  FSessAuth := False;
  FRoot := '';
...
procedure TXmppClientConnection.Login;
begin
...
procedure TXmppClientConnection.Logout;
begin
...
procedure TXmppClientConnection.DoOnConnected(Sender:TObject);
begin
  SetDefaultVal;
  FRoomRoster.Clear;
...
procedure TXmppClientConnection.DoOnDebugXML(Sender: TObject; Value:string);
begin
  if Pos('<',Value)>0 then
  begin
{$IFDEF DEBUG_XML}
...
procedure TXmppClientConnection.DoOnDisconnected(Sender:TObject);
begin
  SetDefaultVal;
...
procedure TXmppClientConnection.DoOnError(Sender:TObject;Value:string);
begin
  DoError(Value);
end;

procedure TXmppClientConnection.DoAfterUpgradedToSSL(Sender:TObject);
begin
  SendXMPPHeader(FCurServer);
end;

procedure TXmppClientConnection.DoOnSSLFailed(Sender:TObject;Value:string);
begin
  // what TODO ?
  DoError('SSL connection failed!');
end;

procedure TXmppClientConnection.SendCommand(XML: string);
begin
...
procedure TXmppClientConnection.SendXMPPHeader(AHost:string);
begin
  SendCommand('<stream:stream to="'+AHost+'"  xmlns="jabber:client"'+
       ' xmlns:stream="http://etherx.jabber.org/streams"  version="1.0">');
end;

procedure TXmppClientConnection.SessionResult(sender: TObject; iq: TXMLTag;
  data: Pointer);
begin
...
procedure TXmppClientConnection.DoError(ErrMsg: string);
begin
...
// exodus
function TXmppClientConnection.GetFullTag(AData: string): string;
...
procedure TXmppClientConnection.ProsesData(AData: string);
var
  cp_buff: string;
  fc,frag: string;
begin
...
procedure TXmppClientConnection.ParseTags(AData: string);
var
  c_tag: TXMLTag;
begin
...
procedure TXmppClientConnection.ProsesTags(tag: TXMLTag);
var s:string;
begin
...
procedure TXmppClientConnection.ParsingFeatures(tag: TXMLTag);
var
  x:TXMLTag;
  tl:TXMLTagList;
  st:TStringList;
...
procedure TXmppClientConnection.BindingResource;
var
  biq:TBindIq;
begin
  if FResource='' then
    biq:=TBindIq.Create('set',TJid.create(FHost))
  else
    biq:=TBindIq.Create('set',TJid.create(FHost),FResource);
  IqGrabber.IqGrab.sendiq(biq,Bindresult,nil);
  biq.free;
end;

procedure TXmppClientConnection.BindingSession;
var s:string;
begin
...
procedure TXmppClientConnection.BindResult(sender: TObject; iq: txmltag;
  data: Pointer);
var
  jid:TJid;
  siq:TSessionIq;
...
begin
  tb:=TBindIq.Create;
  tb.Parse(iq);
  if tb.IqType='result' then
  begin
    if Assigned(tb.bind) then
    begin
      jid:=TBind(tb.Bind).Jid;
      fResource:=jid.resource;
      FUser:=jid.user;
    end;
    ...
    siq:=TSessionIq.Create('set',TJID.Create(FHost));
    IqGrabber.IqGrab.SendIq(siq,sessionresult,'');
    siq.Free;
  end
  else if tb.IqType='error' then
  begin
...
procedure TXmppClientConnection.Send(XML: string);
begin
  SendCommand(xml);
end;

procedure TXmppClientConnection.SendAuth(AuthMethod:string);
begin
  SendCommand('<auth xmlns="'+XMLNS_SASL+'" mechanism="'+AuthMethod+'" xmlns:ga="'+
    XMLNS_GOOG+'" ga:client-uses-full-bind-result="true"></auth>');
end;

procedure TXmppClientConnection.SendMD5Auth;
begin
  SendAuth('DIGEST-MD5');
//  SendCommand('<auth xmlns="'+XMLNS_SASL+'" mechanism="DIGEST-MD5" xmlns:ga=""/>');
end;

procedure TXmppClientConnection.SendMD5Response(tag: TXMLTag);
var
  c,resp,s:string;
begin
  c := tag.Data;
...
procedure TXmppClientConnection.SendCramMD5Auth;
begin
  SendAuth('CRAM-MD5');
//  SendCommand('<auth xmlns="'+XMLNS_SASL+'" mechanism="CRAM-MD5"/>');
end;

procedure TXmppClientConnection.SendCramMD5Response(tag: TXMLTag);
var
  c,resp,s:string;
begin
  c := tag.Data;
...
procedure TXmppClientConnection.SendPLAINAuth;
var
  s,buf:string;
begin
...
function TXmppClientConnection.GenerateID: string;
begin
  Inc(FMYID);
...
function TXmppClientConnection.GetJID: string;
begin
  Result := SeparateLeft(FJID,'/');
end;

procedure TXmppClientConnection.IQBeforeLoggedIn(tag: TXMLTag);
var p:TXMLTag;
begin
...
procedure TXmppClientConnection.ParsingIQ(tag: TXMLTag);
var
  ty,iqid,iqfr,
  iqty,iqvar,
  trid:string;
...
begin
  ...
  begin
    ...
    if (q<>nil) then begin
      if (q.Namespace=NS_DISCO_ITEMS) then
      begin
      ...
      if (q.Namespace=NS_DISCO_INFO) then
      begin
        for i:=0 to q.ChildCount-1 do
        begin
          qi := q.ChildTags.Tags[i];
      ...
      if (q.Namespace=NS_IQ_ROSTER) then
      begin
      ...
      if not FPresenceSet then
        if FCurrentID=trid then
          SendCommand('<iq type="get" id="'+GenerateID+'"><query xmlns="'+NS_IQ_ROSTER+'"/></iq>');
...
procedure TXmppClientConnection.ParsingMessage(tag: TXMLTag);
var
  p,x,d:TXMLTag;
  mf,mt,mb,mh,
  fr,ty,_ts:string;
...
procedure TXmppClientConnection.ParsingPresence(tag: TXMLTag);
var
  p,x:TXMLTag;
  pf,pty,
  pid:string;
...
function TXmppClientConnection.IsInRosterRoom(JID: string): Boolean;
var i:integer;
begin
  Result := False;
...
procedure TXmppClientConnection.RemoveFromRosterRoom(JID: string);
var i:integer;
begin
...
procedure TXmppClientConnection.JoinRoom(RoomName: string);
begin
...
procedure TXmppClientConnection.AddToRosterRoom(JID: string);
begin
  if not IsInRosterRoom(JID) then
  begin
    FRoomRoster.Add(JID);
...
function TXmppClientConnection.GetRosterRoomJID(JID: string): string;
var i:integer;
begin
  Result := '';
  for i:=0 to FRoomRoster.Count-1 do begin
...
procedure TXmppClientConnection.LeaveRoom;
begin
  if FRoomName='' then
    Exit;
...
procedure TXmppClientConnection.DoOnTimer(Sender: TObject);
begin
  FTimer.Enabled := False;
  if not FSessAuth then
    Exit;
...
procedure TXmppClientConnection.GetRoomList;
begin
  if (FConference='') then
    Exit;
...
function TXmppClientConnection.GenerateMSGID: string;
begin
  Inc(FMSGID);
  Result := 'msg'+Format('%5.5d', [FMSGID]);
end;

procedure TXmppClientConnection.SendChatMessage(ToJID, MsgText, MsgHtml: string;
...
procedure TXmppClientConnection.SendPersonalMessage(ToJID, MsgText: string);
begin
  SendChatMessage(ToJID,MsgText,MsgText,mtPersonal);
end;

procedure TXmppClientConnection.SendRoomMessage(MsgText: string);
begin
  if FRoomName='' then
...
procedure TXmppClientConnection.ParsingIQRoster(tag:TXMLTag);
var
...
begin
  for i:=0 to tag.ChildTags.Count-1 do begin
    x :=TRosterItem.Create;
    x.Parse(tag.ChildTags[i]);
